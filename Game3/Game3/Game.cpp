...
void Game::initWorld()
{
	if (!this->worldBgTexture.loadFromFile("Textures/world.jpg")) {
		std::cout << "no world texture load" << "\n";
	}

	this->worldBackground.setTexture(this->worldBgTexture);
}
...
void Game::initSFX()
{
	if (!this->bgMusic.openFromFile("SFX/space_theme.ogg"))
		std::cout << "no song load" << "\n";
	bgMusic.setVolume(2.f);
